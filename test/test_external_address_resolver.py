    ...
    return [
        {
            ...
            "style": "style(<bg=#eff6ff><b></b></bg>; <bg=#ffffff></bg>)",
            "data": "data(<u><url=goto:Table_1!List_2!Summary>Go to List_2 summary</url>; List_1 row</u>)",
        },
        {
            "document": "Table_1",
            "table_name": "List_2",
            "sheet_name": "List_2",
            "preamble": '2,1,1,"#000","solid",0,24',
            "style": "style(<bg=#f0fdf4></bg>; <bg=#dcfce7><b></b></bg>)",
...
def _real_name_tables():
    return [
        ...
        },
        {
            ...
            "preamble": '2,1,1,"#000","solid",0,24',
            "style": "style(<bg=#fef3c7></bg>; <bg=#fde68a><b></b></bg>)",
            "data": "data(Assets; <mark=Totals>Balance totals)",
        },
    ]


def _assert_only_logical_goto_inputs(tables):
    data_source = "\n".join(str(table.get("data", "")) for table in tables)

    assert "goto:" in data_source
    assert 'href="' not in data_source
    assert "#tt-" not in data_source
    assert "#List_" not in data_source
    assert "#Report!" not in data_source
    assert "hyperlink" not in data_source.lower()
...
def build_external_resolver_artifacts():
    ...
    xlsx_output = demo_output_path("external_resolver_navigation_full.xlsx")
    ...
    for output in (html_output, xlsx_output, stacked_xlsx_output, pdf_output, multitable_pdf_output):
        if output.exists():
            output.unlink()
...
def test_address_resolver_resolves_table_list_mark():
    # Unit-level resolver test: no HTML/XLSX/PDF links are rendered here.
...
def test_multitable_html_maps_scoped_goto_to_pycells_table_list_mark():
    tables = _tables()
    _assert_only_logical_goto_inputs(tables)
...
def test_multitable_style_overlay_preserves_third_styled_empty_row():
    navigation = _multitable_address_tables()[0]
    ...
    third_cell = model.cells[2][0]

    assert third_cell.raw_expr == "Open customer summary"
    assert third_cell.link.raw == "goto:CRM!Customers!Summary"
    assert third_cell.styles["text-align"] == "left"
    assert third_cell.styles["text-decoration"] == "underline"
    assert third_cell.styles["background-color"] == "#eff6ff"
